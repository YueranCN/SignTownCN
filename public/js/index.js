...
$(document).ready(() => {
  ...
  const IMAGE_STACK = []; // 用于存储用户点击“开始录制”时的画面
  const PREDICTION_IMAGE_STACK = []; // 用于存储经过模型处理后的画面
  const FRAME_KEYPOINTS_TABLE = []; // 用于存储每个画面中检测到的关键点信息
  const RESULT_POSE_STACK = []; // 用于存储每个画面中检测到的姿态信息
  const RESULT_FACE_STACK = []; // 用于存储每个画面中检测到的面部信息
  const RESULT_LEFTHAND_STACK = []; // 用于存储每个画面中检测到的左手信息
  const RESULT_RIGHTHAND_STACK = []; // 用于存储每个画面中检测到的右手信息
  const topFiveResultArr = []; // 用于存储识别结果的前五位，包括手语符号和准确率
  let signingResult = "";
  ...
  const startClassify = () => {
    ...
    const predictionImage = async () => {
      ...
      const text = document.getElementById('result-prediction-text')
      for (const item of topFiveResultArr) {
        if (item.sign === window.recoil.sign && item.acc >= 0.5) {
          text.style.color = '#67C23A'
          text.textContent = '做的真棒！'
          break;
        } else {
          text.style.color = '#F56C6C'
          text.textContent = '咦！你的手语做错了'
          break;
        }
      }
